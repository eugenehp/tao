...
use objc::{ declare::ClassDecl, runtime::{ Class, Object, Sel, BOOL, NO, YES } };
...
macro_rules! add_property {
    ...
    (
        $decl:ident,
        $name:ident: $t:ty,
        $setter_name:ident: $capability:expr,
        $err:expr;
        | $object:ident | $after_set:expr,
        $getter_name:ident,
    ) => {
        {
...
unsafe fn get_view_class(root_view_class: &'static Class) -> &'static Class {
    ...
    classes.entry(root_view_class).or_insert_with(move || {
        ...
        extern "C" fn draw_rect(object: &Object, _: Sel, rect: CGRect) {
            unsafe {
                ...
                app_state::handle_nonuser_events(
                    std::iter
                        ::once(
                            EventWrapper::StaticEvent(
                                Event::RedrawRequested(RootWindowId(window.into()))
                            )
                        )
                        .chain(
                            std::iter::once(EventWrapper::StaticEvent(Event::RedrawEventsCleared))
                        )
                );
        ...
        extern "C" fn set_content_scale_factor(
          ...
          setContentScaleFactor: untrusted_scale_factor
                ...
                app_state::handle_nonuser_events(
                    std::iter
                        ::once(
                            EventWrapper::EventProxy(EventProxy::DpiChangedProxy {
                                window_id: window,
                                scale_factor,
                                suggested_size: size,
                            })
            ...
            unsafe {
                ...
                loop {
                    ...
                    let force = if os_supports_force {
                        ...
                        let touch_capability: UIForceTouchCapability = msg_send![
                            trait_collection,
                        ...
                        if touch_capability == UIForceTouchCapability::Available {
                            ...
                            let max_possible_force: CGFloat = msg_send![
                                touch,
        ...
        let mut decl = ClassDecl::new(&format!("TaoUIView{}", ID), root_view_class).expect(
            "Failed to declare class `TaoUIView`"
        );
        ID += 1;
...
unsafe fn get_view_controller_class() -> &'static Class {
    ...
    if CLASS.is_none() {
        ...
        let mut decl = ClassDecl::new("TaoUIViewController", uiviewcontroller_class).expect(
            "Failed to declare class `TaoUIViewController`"
        );
        decl.add_method(
...
unsafe fn get_window_class() -> &'static Class {
    ...
    if CLASS.is_none() {
        ...
        let mut decl = ClassDecl::new("TaoUIWindow", uiwindow_class).expect(
            "Failed to declare class `TaoUIWindow`"
        );
        decl.add_method(sel!(becomeKeyWindow), become_key_window as extern "C" fn(&Object, Sel));
        decl.add_method(sel!(resignKeyWindow), resign_key_window as extern "C" fn(&Object, Sel));
...
pub unsafe fn create_window(
    ...
    match window_attributes.fullscreen {
        Some(Fullscreen::Exclusive(ref video_mode)) => {
            ...
            let () = msg_send![window, setScreen:video_mode.monitor().ui_screen()];
        }
...
pub fn create_delegate_class() {
    ...
    extern "C" fn application_continue(
        ...
        unsafe {
            ...
            app_state::handle_nonuser_event(
                EventWrapper::StaticEvent(Event::Opened { urls: vec![url] })
            );
    ...
    let mut decl = ClassDecl::new("AppDelegate", ui_responder).expect(
        "Failed to declare class `AppDelegate`"
    );
    ...
    unsafe {
        ...
        // Unlike macOS with `application:openURLs:` iOS has a different configuration.
        // Do not confuse it with the universal links
        ...
        decl.add_method(
            ...
            application_open_urls as extern "C" fn(&Object, Sel, id, id, id)
        );

        decl.register();
    }

    extern "C" fn application_open_urls(_: &Object, _: Sel, _: id, url: id, _: id) -> () {
        trace!("Trigger `application:openURLs:`");

        let url = unsafe {
            url::Url
                ::parse(&CStr::from_ptr(url.absoluteString().UTF8String()).to_string_lossy())
                .unwrap()
        };
        trace!("Get `application:openURLs:` URLs: {:?}", url);
        // AppState::open_urls(urls);

        unsafe {
            app_state::handle_nonuser_events(
                std::iter::once(EventWrapper::StaticEvent(Event::Opened { urls: vec![url] }))
            );
        }
